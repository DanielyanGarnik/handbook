...
| 3 | Kickoff | Репозитории, первый коммит от каждого, **walking skeleton на школьном сервере**. Python и git даются здесь же, через сам скелет |
...
- **Handover не отрезается ни при каких обстоятельствах.** При отставании режется объём v1.0,
  не передача.
- **Языку учим на ходу.** Python знают 2–3 человека из 11, C++ — ещё 2–3, большинство не знает
  ни одного (см. [TEAM-SKILLS.md](TEAM-SKILLS.md)). Отдельной недели «сначала язык, потом проект»
  в плане нет и быть не может — Python и git даются на неделе 3 через walking skeleton, то есть
  ровно в объёме, нужном, чтобы довести свою программу до сервера. Это тот же принцип
  «инструмент вводится в день боли», применённый к языку.
- Из таблицы «боль → инструмент» (§5) реально успеваем строки 0–6 и 9. Строки 7, 8, 10 —
...
| Проверить: python3 --version, git, ssh, cron/systemd, свободный порт, бэкапы | там же |
...
|---|---|---|---|
| 0 | «Я не знаю языка, я не могу начать» | Python — через walking skeleton, не отдельным курсом | 3 |
